...
impl ApplicationDescription {
    pub fn to_json(&self) -> JsonValue {
        JsonValue::Object(vec![
            ...
            ("deployment_mode".to_string(), JsonValue::String(self.deployment_mode.clone())),
            ("contract_fingerprint".to_string(), JsonValue::String(self.contract_fingerprint.clone())),
            ("surface_fingerprint".to_string(), JsonValue::String(self.surface_fingerprint.clone())),
            ("started_at".to_string(), JsonValue::Number(
                self.started_at
                    .duration_since(SystemTime::UNIX_EPOCH)
                    .unwrap_or_default()
                    .as_secs() as f64
            )),
            ("uptime_seconds".to_string(), JsonValue::Number(self.uptime_seconds as f64)),
        ])
    }
}
    ...
    pub fn to_json(&self) -> JsonValue {
        match self {
            ...
            Self::CapabilityRequired(cap) => JsonValue::Object(vec![
                ("type".to_string(), JsonValue::String("capability_required".to_string())),
...
impl RouteDescription {
    pub fn to_json(&self) -> JsonValue {
        JsonValue::Object(vec![
            ...
            ("methods".to_string(), JsonValue::Array(
                self.methods.iter().map(|m| JsonValue::String(m.clone())).collect()
            )),
            ("protection".to_string(), self.protection.to_json()),
            ("aliases".to_string(), JsonValue::Array(
                self.aliases.iter().map(|a| JsonValue::String(a.clone())).collect()
            )),
        ])
    }
}
