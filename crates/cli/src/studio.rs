...
pub fn run(args: &[String]) -> Result<Output, CliError> {
    ...
    let (mailport_url, mailport_api_key) = serve::mailport_environment(&root);
...
<h2>Development users</h2><section class="card"><p class="muted">Local-only accounts generated for this repository. Credentials are stored in <code>.authboundry/development.json</code> and excluded from git.</p><table><thead><tr><td>User</td><td>Role</td><td>Password</td></tr></thead><tbody>{account_rows}</tbody></table><h3>Create user</h3><form id="create-user"><label>Username<input name="username" required></label><label>Email <small>Optional</small><input name="email" type="email" placeholder="name@example.com"></label><label>Password <small>Required · at least 12 characters</small><input name="password" type="password" minlength="12" autocomplete="new-password" required></label><label>Role<select name="role"><option value="user">user</option><option value="admin">admin</option></select></label><br><button type="submit">Create user</button><span id="create-user-status" class="muted"></span></form></section>
